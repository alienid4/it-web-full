# IT 每日自動巡檢系統 — 完整開發規格手冊

> ⚠️ **本文件為 v3.4.2 時代產物（2026-04 中）**
>
> 目前系統已進到 **v3.11.2**，涵蓋更多模組（Windows SSH、SNMP、AS400、
> 帳號盤點、Patch 管理、nmon 效能、CIO 儀表板、TWGCB 強化、Filter Rules、
> Feature Flags 等）。本規格**未完整涵蓋 v3.5-v3.11 新增功能**。
>
> **新接手的工程師請先看**：
> 1. [`DEVELOPER.md`](./DEVELOPER.md) — 架構概覽、routes/services 盤點、開發流程
> 2. [`CHANGELOG.md`](./CHANGELOG.md) — 每版變動細節
> 3. [`AI/data/RUNBOOK.md`](./AI/data/RUNBOOK.md) — 常見問題排錯
> 4. [`AI/data/ITAGENT_MANUAL.md`](./AI/data/ITAGENT_MANUAL.md) — 運維手冊
>
> 本文件下方內容仍對 **Phase 1 核心功能**（基本巡檢、dashboard、報告）有完整說明，
> 作為歷史參考保留。規劃新功能時請以實際 code 為準，不要盲目照本文件實作。

---

## 目的（原文）

本文件是「一模一樣重建」整套巡檢系統的**唯一參考**。
...
- 備份現有檔案至 `/var/backups/inspection/pre_security_audit_{DATE}/`
...
```bash
mkdir -p /opt/inspection/container/mongodb_data
podman run -d --name mongodb \
  -p 127.0.0.1:27017:27017 \
  -v /opt/inspection/container/mongodb_data:/data/db:Z \
  --restart=always \
...
```bash
30 6 * * *  /opt/inspection/run_inspection.sh >> logs/cron.log 2>&1
30 13 * * * /opt/inspection/run_inspection.sh >> logs/cron.log 2>&1
30 17 * * * /opt/inspection/run_inspection.sh >> logs/cron.log 2>&1
```
...
- **變更前自動備份**：`tar czf /var/backups/inspection/INSPECTION_HOME_$(date).tar.gz`
- **保留數量**：最近 3 份，舊的自動刪除
...
| Ansible Vault | `/opt/inspection/.vault_pass` |
| Ansible Inventory | `/opt/inspection/ansible/inventory/hosts.yml` |
| 巡檢日誌 | `/opt/inspection/logs/` |
| 巡檢報告 | `/opt/inspection/data/reports/` |
| 系統設定 | `/opt/inspection/data/settings.json` |
| 版本資訊 | `/opt/inspection/data/version.json` |
