...
# （舊版有 SECANSIBLE 硬編碼，v3.11.4.0 改成從 hosts_config.json connection=local 欄位動態產生）
...
def build_inventory(hosts):
    ...
    # 加入 os_group 結構
    # v3.17.19.14: 修 windows.children.windows 自己包自己 (ansible 報 "can't add group to itself")
    for big_group, sub_groups in os_groups.items():
        # 如果只有一個 sub_group 而且名字跟 big_group 一樣 (例如 windows -> windows), 直接放 hosts 不嵌套
        if len(sub_groups) == 1 and big_group in sub_groups:
            children[big_group] = sub_groups[big_group]
        else:
            children[big_group] = {"children": {}}
            for sub_group, sub_data in sub_groups.items():
                # 子群組同名也跳過嵌套 (例如混合場景下的 windows.windows)
                if sub_group == big_group:
                    if "hosts" not in children[big_group]:
                        children[big_group]["hosts"] = {}
                    children[big_group]["hosts"].update(sub_data.get("hosts", {}))
                else:
    ...
    # ── 4. management 群組（由 hosts_config.json 中 connection=local 的主機組成）
    mgmt_hosts = {}
    for host in hosts:
        conn = (host.get("connection") or host.get("ansible_connection") or "").strip().lower()
        if conn == "local":
            mgmt_hosts[host["hostname"]] = {
                "ansible_host": host.get("ip", "127.0.0.1"),
                "ansible_connection": "local",
            }
    if mgmt_hosts:
        children["management"] = {"hosts": mgmt_hosts}
...
def build_host_vars(host):
    ...
    # connection=local → 本機不走 SSH（給 control node 自己用）
    conn = (host.get("connection") or host.get("ansible_connection") or "").strip().lower()
    if conn == "local":
        vars["ansible_connection"] = "local"

    # v3.11.6.0: SSH user 從 host 欄位讀（ssh_user 或 ansible_user）
    # 沒設就用 group_vars/all.yml 的預設（sysinfra）
    ssh_user = host.get("ssh_user") or host.get("ansible_user")
    if ssh_user:
        vars["ansible_user"] = ssh_user

    # SSH port 同理
    ssh_port = host.get("ssh_port") or host.get("ansible_port")
    if ssh_port:
        try:
            vars["ansible_port"] = int(ssh_port)
        except (TypeError, ValueError):
            pass

    # SSH private key 檔案路徑（可選）
    ssh_key = host.get("ssh_key") or host.get("ansible_ssh_private_key_file")
    if ssh_key:
        vars["ansible_ssh_private_key_file"] = ssh_key
