...
if ! echo "$CUR_VER" | grep -qE '^3\.17\.(18|19)\.'; then
    echo "[WARN] Expected baseline 3.17.18.x or 3.17.19.x, found $CUR_VER"
    echo "       建議先升到 3.17.18.0 再裝此 bundle. 強制繼續? (y/N)"
    read -r ans
    [ "$ans" = "y" ] || [ "$ans" = "Y" ] || exit 1
fi
