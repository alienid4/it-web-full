...
def sync_systems_from_hosts(rows, source="csv_import", actor="system"):
    """
    從 hosts import 結果自動同步 dependency_systems。

    rows: list of dict, 每筆 {"hostname": str, "system_id": str, "metadata": {...}}
        - hostname 必填
        - system_id 為「資產名稱」(中文業務系統名直接當 PK), 空字串/None 表示這台不歸屬任何系統 (跳過)
        - metadata 選填: {apid, group, infra, owner} 等資產表帶過來的輔助欄

    行為:
        1. 整理出 hostname → new_system_id 的對映
        2. 找出 dependency_systems 裡 host_refs 含這些 hostname 的舊紀錄
        3. 對每筆: 若舊 system != new_system → 從舊 system.host_refs 移除這 hostname
        4. upsert 新 system_id: 不存在則建 (帶 metadata 預設值), 已存在則 $addToSet host_refs
        5. metadata 異動 (apid/group/infra/owner) 也一併寫進 system.metadata

    回傳: {"systems_created": N, "systems_updated": N, "hosts_moved": N, "skipped": N}
    """
    sys_col = get_collection("dependency_systems")
    now = datetime.utcnow()
    ...
    for r in rows or []:
        ...
        if not h or not sid:
            skipped += 1
            continue
        valid_rows.append({"hostname": h, "system_id": sid, "metadata": r.get("metadata") or {}})

    if not valid_rows:
        return {"systems_created": 0, "systems_updated": 0, "hosts_moved": 0, "skipped": skipped}

    hostnames = [r["hostname"] for r in valid_rows]

    old_owners = {}
    for s in sys_col.find({"host_refs": {"$in": hostnames}}, {"system_id": 1, "host_refs": 1}):
        for h in s.get("host_refs", []):
            if h in hostnames:
                old_owners[h] = s["system_id"]

    moved = 0
    for r in valid_rows:
        h, new_sid = r["hostname"], r["system_id"]
        old_sid = old_owners.get(h)
        if old_sid and old_sid != new_sid:
            sys_col.update_one(
                {"system_id": old_sid},
                {"$pull": {"host_refs": h}, "$set": {"updated_at": now, "updated_by": actor}}
            )
            moved += 1
    ...
    by_sid = {}
    for r in valid_rows:
        by_sid.setdefault(r["system_id"], {"hosts": [], "metadata": {}})
        by_sid[r["system_id"]]["hosts"].append(r["hostname"])
        by_sid[r["system_id"]]["metadata"].update(r["metadata"] or {})

    for sid, payload in by_sid.items():
        existing = sys_col.find_one({"system_id": sid})
        meta = payload["metadata"]
        owner = meta.pop("owner", "") if meta else ""

        if existing:
            update_doc = {
                "$addToSet": {"host_refs": {"$each": payload["hosts"]}},
                "$set": {"updated_at": now, "updated_by": actor, "source": source},
            }
            if meta:
            ...
            if owner and not existing.get("owner"):
                update_doc["$set"]["owner"] = owner
            sys_col.update_one({"system_id": sid}, update_doc)
            updated += 1
        else:
            sys_col.insert_one({
                "system_id": sid,
                ...
                "owner": owner,
                "ap_owner_email": "",
                "host_refs": list(set(payload["hosts"])),
                "external": False,
                "metadata": meta or {},
                "source": source,
                "created_at": now,
    ...
    return {"systems_created": created, "systems_updated": updated, "hosts_moved": moved, "skipped": skipped}
...
def topology(center=None, depth=2, limit=200, view="system"):
    """根據 view 切換 3 種節點型別 (system/host/ip), 共用同一份 dependency_relations 資料"""
    view = (view or "system").lower()
    if view == "host":
