...
function runHealthCheck() {
  fetch("/api/admin/health-check").then(function(r){return r.json();}).then(function(res) {
    if (!res.success) { alert('健康檢查失敗: ' + (res.error||'')); return; }
    var lines = ['=== 系統健康檢查 ==='];
    lines.push('整體: ' + (res.all_ok ? '✓ 全部 OK' : '✗ '+res.failed_count+' 個 endpoint 異常'));
    lines.push('');
    res.checks.forEach(function(c) {
      var icon = c.ok ? '✓' : '✗';
      lines.push(icon + ' ' + c.label + ' [' + c.path + ']');
      lines.push('   HTTP=' + c.http + '  json.success=' + c.json_success);
      if (!c.ok) lines.push('   ' + c.preview);
    });
    alert(lines.join('\n'));
  }).catch(function(e){ alert('健康檢查 API 錯誤: ' + e.message); });
}
