...
function verifyNmonDeployment() {
  ...
  fetch('/api/nmon/verify').then(function(r){return r.json();}).then(function(res){
    ...
    html += '<thead><tr style="background:#f3f4f6;">' +
      ...
      '<th style="padding:6px;text-align:center;width:50px;" title="cron 排程是否設好">cron</th>' +
    ...
    hosts.forEach(function(h){
      ...
      if (h.cron_line) detailParts.push('<code style="font-size:10px;color:#666;">cron: ' + h.cron_line.replace(/</g,'&lt;') + '</code>');
      ...
      html += '<tr style="border-bottom:1px solid #eee;">' +
        ...
        '<td style="padding:6px;text-align:center;">' + ok(h.cron_ok) + '</td>' +
