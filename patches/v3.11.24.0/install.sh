...
BACKUP_DIR="/var/backups/inspection/pre_v3.11.24.0_${TS}"
...
cp "$SRC_DIR/data/version.json" "$HOME_DIR/data/version.json" && chown "$OWNER" "$HOME_DIR/data/version.json" && ok "version.json → 3.11.24.0"
...
case "$HTTP_CODE" in
    200|302|401) ok "API $HTTP_CODE" ;;
    *)           echo -e "  ${YELLOW}WARN${NC} API $HTTP_CODE" ;;
esac

echo ""
echo -e "${GREEN}v3.11.24.0 安裝完成!${NC}"
echo "測試: Ctrl+Shift+R /report → 深度檢查 → 展開 WARN 項目 → 應見 📋指令 / ⚠️風險 / ✅驗證 3 區塊"
echo ""
echo "Rollback:"
