...
# v3.17.19.13: static 檔案瀏覽器 cache 1 小時 (cache busting 透過 ?v=APP_VER)
...
def add_security_headers(response):
    ...
    # static 檔案 1 小時 browser cache (cache busting 用 ?v=APP_VER, 改檔即生效)
    ...
    response.headers["Content-Security-Policy"] = "default-src 'self' 'unsafe-inline' 'unsafe-eval' https://fonts.googleapis.com https://fonts.gstatic.com https://cdn.jsdelivr.net https://static.cloudflareinsights.com; img-src 'self' data: https:; connect-src 'self' https://cloudflareinsights.com https://cdn.jsdelivr.net"
...
def _load_app_version():
    try:
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "data", "version.json"), encoding="utf-8") as _f:
            j = _json.load(_f)
            return (j.get("version", "dev"), j.get("updated_at", ""))
    except Exception:
        return ("dev", "")
_APP_VER, _APP_UPDATED_AT = _load_app_version()

@app.context_processor
def _inject_app_version():
    return {"APP_VER": _APP_VER, "APP_UPDATED_AT": _APP_UPDATED_AT}
