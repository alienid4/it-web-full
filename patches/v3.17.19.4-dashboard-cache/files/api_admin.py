...
@admin_required
def system_info():
    """v3.17.19.4: TTL 60s 快取 + 平行 6 個指令 (主要是省 ansible --version 400ms)."""
    def _build():
        from concurrent.futures import ThreadPoolExecutor
        def cmd(c):
            try:
                return subprocess.run(c, capture_output=True, text=True, timeout=8, shell=isinstance(c, str)).stdout.strip()
            except Exception:
                return "N/A"
        tasks = {
        ...
        with ThreadPoolExecutor(max_workers=6) as ex:
            futures = {k: ex.submit(cmd, v) for k, v in tasks.items()}
