...
CHECK_CMD = (
    'echo "=CRON="; sudo crontab -l 2>/dev/null | grep -E "nmon|topas_nmon"; '
    'echo "=BIN="; (command -v nmon 2>/dev/null || command -v topas_nmon 2>/dev/null || echo NOT_INSTALLED); '
    'echo "=LOG="; ls -lt /var/log/nmon/daily/*.nmon 2>/dev/null | head -3; '
...
def main():
    ...
    for h in enabled:
        ...
        cron_lines_raw = [l.strip() for l in sec["cron"].strip().split("\n") if l.strip()]
        cron_daily_ok = any("-c 1440" in l for l in cron_lines_raw)
        cron_monthly_ok = any("-c 96" in l for l in cron_lines_raw)
        cron_ok = cron_daily_ok and cron_monthly_ok
        ...
        item.update({
            ...
            "cron_lines": cron_lines_raw,
            "cron_line": cron_lines_raw[0] if cron_lines_raw else "",
            "cron_daily_ok": cron_daily_ok,
            "cron_monthly_ok": cron_monthly_ok,
        ...
        if cron_ok and binary_ok:
            if daily_today > 0 or log_ok:
                ...
                item["status"] = "PARTIAL"
                item["detail"] = "cron+binary 都在但尚未採樣 (剛部署後等隔天 00:00 cron 執行)"
                summary["partial"] += 1
        else:
            item["status"] = "FAIL"
            issues = []
            if not cron_ok:
                if not cron_daily_ok: issues.append("缺日報 cron (-c 1440)")
                if not cron_monthly_ok: issues.append("缺月報 cron (-c 96)")
            if not binary_ok:
