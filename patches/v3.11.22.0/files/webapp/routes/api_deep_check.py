...
INSPECTION_HOME = os.environ.get("INSPECTION_HOME", "/seclog/AI/inspection")
...
def run_deep_check():
    ...
    # Controller 自己無法做 (sudo 密碼問題); 請直接 SSH 進 controller 手動跑 mod_troubleshoot.sh -m
    if hostname == _CONTROLLER_HOSTNAME:
        return jsonify({"success": False, "error": f"無法對巡檢系統主機 ({hostname}) 自身執行深度檢查。請直接 SSH 到該主機手動執行。"}), 400
    ...
    extra_vars = {"job_timestamp": job_ts}
