...
OS_MAP = {
    # Windows (長 key 在前; substring match 模式下避免短 key 提早 match)
    ...
    "Red Hat": "rhel",
    "RedHat": "rhel",        # v3.17.12.0+: 相容 "RedHat 9.4" 沒空格寫法
    "RHEL": "rhel",
    "Oracle Linux": "rhel",
    # CentOS / Rocky / Alma
    "CentOS Linux": "centos",
...
def detect_os_group(os_str):
    ...
    v3.17.12.0+: 改成 long-first 比對, 避免短 key (如 'AIX') 在 'Linux AIX-helper' 之類字串中誤命中.
    """
    if not os_str:
        return "unknown"
    s_low = os_str.lower()
    for key in sorted(OS_MAP.keys(), key=len, reverse=True):
        if key.lower() in s_low:
            return OS_MAP[key]
    return "unknown"
