...
v3.17.12.0+: 加 family_to_group / infer_os_group → NMON 排程要看 os_group
...
# v3.17.12.0+: canonical Family → 內部 os_group key (給 NMON / TWGCB 排程白名單用)
_FAMILY_TO_GROUP = {
    "RHEL": "rhel",
    "CentOS": "centos",
    "Rocky Linux": "rocky",
...
def family_to_group(family):
    """canonical family → os_group. 例: 'RHEL' → 'rhel'. 找不到回 ''"""
    if not family:
        return ""
    return _FAMILY_TO_GROUP.get(family, "")


def infer_os_group(os_str):
    """從原始 OS 字串直接推 os_group. 例: 'RedHat 9.4' → 'rhel'"""
    if not os_str:
        return ""
    fam, _ = parse_os(os_str)
    return family_to_group(fam)
