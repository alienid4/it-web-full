...
function loadTab(tab) {
  ...
  var loaders = {
    ...
    "diagnostic": function(){ if (typeof loadDiagnosticTab === "function") loadDiagnosticTab(); },
  };
...
function loadDashboardTab() {
  // v3.17.19.2: 防靜默失敗 - 任何 endpoint 異常顯示明確錯誤
  fetch("/api/admin/system/status").then(function(r){
    if (!r.ok) throw new Error('HTTP ' + r.status);
    return r.json();
  }).then(function(res) {
    if (!res.success) {
      document.getElementById("sys-kpi").innerHTML = '<div style="color:var(--red);padding:10px;">⚠ 系統狀態載入失敗：'+(res.error||'unknown')+'</div>';
      return;
    }
    var d = res.data;
    ...
    document.getElementById("sys-kpi").innerHTML = '<div style="color:var(--red);padding:10px;">⚠ 系統狀態 API 錯誤：'+e.message+' [<a href="javascript:runHealthCheck()" style="color:var(--primary);">診斷</a>]</div>';
  });
  fetch("/api/admin/system/info").then(function(r){
    if (!r.ok) throw new Error('HTTP ' + r.status);
    return r.json();
  }).then(function(res) {
    if (!res.success) {
      document.getElementById("sys-info").innerHTML = '<div style="color:var(--red);">⚠ '+(res.error||'unknown')+'</div>';
      return;
    }
    var d = res.data;
    ...
    document.getElementById("sys-info").innerHTML = '<div style="color:var(--red);">⚠ 載入失敗：'+e.message+'</div>';
  });
  loadOnlineUsers();
}

// v3.17.19.2: 一鍵端到端健康檢查 (使用者主動診斷工具)
function runHealthCheck() {
  // 簡化: 直接跳到診斷 tab
  var btn = document.querySelector('.admin-tab[data-tab="diagnostic"]');
  if (btn) btn.click();
  setTimeout(function(){ runDiagnostic('quick'); }, 300);
}

// v3.17.19.16: 系統診斷 (sanitized + downloadable)
function loadDiagnosticTab() {
  // 預設不主動跑, 等使用者點按鈕
  var sum = document.getElementById('diag-summary');
  var res = document.getElementById('diag-results');
  if (sum) sum.style.display = 'none';
  if (res && !res.innerHTML) res.innerHTML = '<div style="color:var(--c3);padding:20px;text-align:center;">點上方按鈕開始檢查</div>';
}

function runDiagnostic(mode) {
  var resDiv = document.getElementById('diag-results');
  var sumDiv = document.getElementById('diag-summary');
  var timeSpan = document.getElementById('diag-time');
  resDiv.innerHTML = '<div style="color:var(--c3);padding:20px;text-align:center;">⏳ ' + (mode === 'deep' ? '深度' : '快速') + '檢查中...</div>';
  sumDiv.style.display = 'none';
  timeSpan.textContent = '';
  var t0 = Date.now();
  fetch('/api/admin/diagnostic/run', {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({mode: mode})
  }).then(function(r){
    if (!r.ok) return r.text().then(function(t){ throw new Error('HTTP '+r.status+': '+t.substring(0,150)); });
    return r.json();
  }).then(function(res) {
    var elapsed = ((Date.now() - t0) / 1000).toFixed(1);
    if (!res.success) { resDiv.innerHTML = '<div style="color:var(--red);">錯誤: '+(res.error||'')+'</div>'; return; }
    var s = res.summary;
    var allOK = s.fail === 0 && s.warn === 0;
    sumDiv.style.display = '';
    sumDiv.innerHTML =
      '<strong>總結 (' + (mode === 'deep' ? '深度' : '快速') + '):</strong> ' +
      '<span style="color:var(--g1);font-weight:600;">✓ ' + s.pass + ' PASS</span>' +
      (s.warn ? ' <span style="color:var(--orange);font-weight:600;"> | ⚠ ' + s.warn + ' WARN</span>' : '') +
      (s.fail ? ' <span style="color:var(--red);font-weight:600;"> | ✗ ' + s.fail + ' FAIL</span>' : '') +
      (allOK ? ' <span style="color:var(--g1);">  系統健康</span>' : '') +
      '<br><span style="font-size:11px;color:var(--c3);">耗時 ' + elapsed + 's | 已自動 sanitize 敏感資訊</span>';
    timeSpan.textContent = res.generated_at.replace('T',' ').split('.')[0];
    ...
    res.sections.forEach(function(sec) {
      ...
      html += '<div style="background:var(--bg2);padding:8px 12px;font-weight:600;font-size:13px;">' + sec.title + '</div>';
      html += '<table style="width:100%;border-collapse:collapse;font-size:12px;">';
      sec.items.forEach(function(it) {
        var icon = {'ok':'✓','warn':'⚠','fail':'✗'}[it.status] || '?';
        var color = {'ok':'var(--g1)','warn':'var(--orange)','fail':'var(--red)'}[it.status] || 'var(--c3)';
        html += '<tr style="border-top:1px solid var(--border);">';
        html += '<td style="padding:5px 12px;width:24px;color:'+color+';font-weight:600;">'+icon+'</td>';
        html += '<td style="padding:5px 12px;">'+it.label+'</td>';
        html += '<td style="padding:5px 12px;color:var(--c2);">'+it.value+'</td>';
        html += '</tr>';
      });
      html += '</table></div>';
    });
    ...
    resDiv.innerHTML = '<div style="color:var(--red);padding:10px;">診斷失敗: '+e.message+'</div>';
  });
}

function downloadDiagnostic(mode) {
  var btn = event.target;
  var orig = btn.textContent;
  btn.disabled = true; btn.textContent = '產生中...';
  fetch('/api/admin/diagnostic/download', {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({mode: mode})
  }).then(function(r){
    ...
    a.href = url;
    a.download = 'inspection-diag-' + (new Date()).toISOString().replace(/[:T]/g,'-').split('.')[0] + '-' + mode + '.txt';
...
function loadHostsTab() {
  fetch("/api/hosts?per_page=200").then(function(r){return r.json();}).then(function(res) {
    ...
    data.forEach(function(h) {
      ...
      html += '<button class="btn btn-sm btn-danger" onclick="deleteHostWithOptions(\''+hn+'\')">刪除</button>';
      html += '</td></tr>';
    });
    html += '</tbody></table>';
...
// v3.17.19.1: 刪除主機 + 可選清歷史
function deleteHostWithOptions(hostname) {
  var msg = "刪除主機「" + hostname + "」？\n\n" +
            "✓ 從 hosts collection 移除\n" +
            "✓ 自動重建 Ansible Inventory\n\n" +
            "OK = 只刪主機（保留巡檢歷史）\n" +
            "Cancel = 取消";
  if (!confirm(msg)) return;
  var purge = confirm("是否同時清掉相關歷史資料？\n\n" +
                      "影響的 collection：\n" +
                      "  inspections / nmon_daily / nmon_timeseries\n" +
                      "  account_audit / twgcb_results / host_packages\n\n" +
                      "OK = 清歷史（永久刪除）\n" +
                      "Cancel = 保留歷史");
  var url = "/api/admin/hosts/" + encodeURIComponent(hostname) + (purge ? "?purge=1" : "");
  fetch(url, {method:"DELETE"})
    ...
    .then(function(res) {
      ...
      var detail = res.message || "已刪除";
      if (res.inventory) detail += "\n• " + res.inventory;
      if (purge && res.purged) {
        var p = res.purged;
        var purgeLines = Object.keys(p).filter(function(k){ return p[k] > 0; })
          .map(function(k){ return "  " + k + ": " + p[k]; });
        if (purgeLines.length) detail += "\n• 清掉歷史:\n" + purgeLines.join("\n");
      }
      alert("✓ " + detail);
      _tabLoaded.hosts = false;
      loadHostsTab();
...
// ===== v3.17.19.0: Batch Onboard (一頁完成 candidate pool) =====
var _boCandidates = [];
...
function showBatchOnboardModal() {
  ...
  var st = document.getElementById('bo-scan-status'); if (st) st.textContent = '';
  var sr = document.getElementById('bo-scan-result'); if (sr) sr.style.display = 'none';
...
function boCandKey(h) { return (h.hostname || '').toLowerCase() + '|' + (h.ip || ''); }
function boAddToCandidates(arr) {
  var existing = {};
  _boCandidates.forEach(function(c){ existing[boCandKey(c)] = true; });
  var added = 0;
  arr.forEach(function(h) {
    if (!h.hostname && !h.ip) return;
    var k = boCandKey(h);
...
function boRenderCandidates() {
  var tbody = document.getElementById('bo-cand-tbody');
  if (!tbody) return;
  var empty = document.getElementById('bo-cand-empty');
  tbody.innerHTML = '';
  document.getElementById('bo-cand-count').textContent = '(' + _boCandidates.length + ')';
  if (empty) empty.style.display = _boCandidates.length ? 'none' : '';
  var checkedCount = _boCandidates.filter(function(c){ return c.checked; }).length;
  document.getElementById('bo-start-count').textContent = checkedCount;
  var osColors = {linux:'#4AB234', aix:'#9C27B0', windows:'#0078D4', as400:'#333', unknown:'#999', '':'#bbb'};
  _boCandidates.forEach(function(c, idx) {
    var color = osColors[c.os_group || ''] || '#999';
    var osBadge = c.os_hint
      ? '<span style="background:'+color+';color:white;font-size:9px;padding:1px 5px;border-radius:3px;">'+c.os_hint+'</span>'
      : '<span style="color:var(--c4);font-size:10px;">未偵測</span>';
    var srcLabel = {paste:'貼清單', single:'單台', csv:'CSV', scan:'掃描'}[c.source] || '手動';
    var tr = document.createElement('tr');
    tr.innerHTML =
      '<td style="padding:3px 6px;border-bottom:1px solid var(--border);text-align:center;">' +
        '<input type="checkbox" '+(c.checked?'checked':'')+' onchange="boCandToggle('+idx+', this.checked)"></td>' +
      '<td style="padding:3px 6px;border-bottom:1px solid var(--border);">'+c.hostname+'</td>' +
      '<td style="padding:3px 6px;border-bottom:1px solid var(--border);font-family:monospace;">'+(c.ip||'-')+'</td>' +
      '<td style="padding:3px 6px;border-bottom:1px solid var(--border);">'+osBadge+'</td>' +
      '<td style="padding:3px 6px;border-bottom:1px solid var(--border);font-size:10px;color:var(--c3);">'+srcLabel+'</td>' +
      '<td style="padding:3px 6px;border-bottom:1px solid var(--border);text-align:center;">' +
        '<button onclick="boCandRemove('+idx+')" style="border:none;background:none;color:#c00;cursor:pointer;font-size:14px;line-height:1;">✕</button></td>';
    tbody.appendChild(tr);
  });
}
function boCandToggle(idx, checked) {
  if (_boCandidates[idx]) _boCandidates[idx].checked = checked;
  boRenderCandidates();
}
function boCandRemove(idx) {
  _boCandidates.splice(idx, 1);
  boRenderCandidates();
}
function boCandToggleAll(state) {
  _boCandidates.forEach(function(c){ c.checked = !!state; });
  boRenderCandidates();
}
function boCandClear() {
  if (!_boCandidates.length) return;
  if (!confirm('清空候選清單？('+_boCandidates.length+' 台)')) return;
  _boCandidates = [];
  boRenderCandidates();
}
function boAddSingle() {
  var hn = (document.getElementById('bo-single-hn').value || '').trim();
  var ip = (document.getElementById('bo-single-ip').value || '').trim();
  if (!hn) { document.getElementById('bo-single-hn').focus(); return; }
  boAddToCandidates([{hostname: hn, ip: ip, source: 'single'}]);
  document.getElementById('bo-single-hn').value = '';
  document.getElementById('bo-single-ip').value = '';
  document.getElementById('bo-single-hn').focus();
}
function boAddPasted() {
  var raw = document.getElementById('bo-paste-input').value;
  var arr = [];
  raw.split('\n').forEach(function(line) {
    var parts = line.trim().split(/\s+/);
    if (parts[0]) arr.push({hostname: parts[0], ip: parts[1] || '', source: 'paste'});
  });
  if (!arr.length) { alert('貼清單區是空的'); return; }
  var added = boAddToCandidates(arr);
  document.getElementById('bo-paste-input').value = '';
  if (added < arr.length) alert('加入 '+added+' 台（'+(arr.length-added)+' 台已在候選清單）');
}
function boCsvSelected(input) {
  ...
  reader.onload = function(e) {
    ...
    if (!lines.length) { document.getElementById('bo-csv-preview').textContent = 'CSV 是空的'; return; }
    ...
    for (var i = 1; i < lines.length; i++) {
      ...
      if (hn) arr.push({hostname: hn, ip: ip, source: 'csv'});
    }
    var added = boAddToCandidates(arr);
    document.getElementById('bo-csv-preview').textContent =
      'CSV 解析 '+arr.length+' 台，加入候選 '+added+' 台';
    input.value = '';
  };
...
function startBatchOnboard() {
  var hosts = _boCandidates.filter(function(c){ return c.checked; })
    .map(function(c){ return {hostname: c.hostname, ip: c.ip}; });
  if (!hosts.length) { alert('請在候選清單勾選至少一台主機'); return; }

  var btn = document.getElementById('bo-start-btn');
  var runInspectEl = document.getElementById('bo-run-inspect');
  var runInspect = runInspectEl ? runInspectEl.checked : true;
  btn.disabled = true; btn.textContent = '上線中...';

  document.getElementById('bo-result').style.display = '';
  ['bo-step-db','bo-step-inv','bo-step-probe','bo-step-inspect'].forEach(function(id){
    var el = document.getElementById(id);
    if (el) { el.style.color = ''; el.innerHTML = el.innerHTML.replace(/^⚙✓✗⏳-\s*/, '⚙ '); }
  });
  document.getElementById('bo-result-tbody').innerHTML = '';

  fetch('/api/admin/hosts/batch-onboard', {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({hosts: hosts, run_inspection: runInspect})
  }).then(function(r){
    // v3.17.19.10: 詳細錯誤回饋, 不再「<!DOCTYPE 不是有效 JSON」這種無資訊訊息
    var ct = r.headers.get('Content-Type') || '';
    if (!r.ok) {
      return r.text().then(function(t){
        throw new Error('HTTP '+r.status+' from '+r.url+'\n回應: '+t.substring(0,200));
      });
    }
    if (ct.indexOf('json') < 0) {
      return r.text().then(function(t){
        throw new Error('預期 JSON 但收到 '+ct+'\nURL: '+r.url+'\n回應前 200 字: '+t.substring(0,200));
      });
    }
    return r.json();
  }).then(function(res) {
    if (!res.success) { alert('上線失敗: '+(res.error||'unknown error')); btn.disabled = false; btn.textContent = '✦ 重試 ✦'; return; }
    // 全部已存在 → 跳出「請先在候選池清掉這些已存在主機」
    if (res.message && res.message.indexOf('均已存在') >= 0) {
      alert(res.message + '\n\n候選清單上的主機已經在系統裡, 不需要再上線.');
      // bug fix: btn 之前 textContent 被改成「上線中」, 內部 span#bo-start-count 已消失. 用 hosts.length 取代.
      btn.disabled = false; btn.textContent = '✦ 一鍵上線勾選的 '+hosts.length+' 台 ✦';
      // 仍走完渲染流程顯示「已存在」狀態
    }
    document.getElementById('bo-step-db').innerHTML = '✓ 加入 DB';
    document.getElementById('bo-step-db').style.color = 'var(--g1)';
    document.getElementById('bo-step-inv').innerHTML = '✓ 重建 Inventory';
    document.getElementById('bo-step-inv').style.color = 'var(--g1)';
    var ok = (res.results||[]).filter(function(r){ return r.probe_status === 'ok'; }).length;
    var fail = (res.results||[]).filter(function(r){ return r.probe_status === 'failed'; }).length;
    document.getElementById('bo-step-probe').innerHTML = '✓ 偵測 OS ('+ok+' 成功 / '+fail+' 失敗)';
    document.getElementById('bo-step-probe').style.color = 'var(--g1)';

    boRenderResults(res.results || []);
    document.getElementById('bo-probe-log').textContent = res.probe_log || '';

    if (runInspect && res.inspection_job_id) {
      document.getElementById('bo-step-inspect').innerHTML = '⏳ 首次巡檢進行中...';
      pollOnboardStatus(res.inspection_job_id, hosts.map(function(h){return h.hostname;}));
    } else {
      document.getElementById('bo-step-inspect').innerHTML = '- 略過首次巡檢';
      document.getElementById('bo-step-inspect').style.color = 'var(--c3)';
      btn.disabled = false; btn.textContent = '✦ 完成 ✦';
    }
    if (typeof loadHostsTab === 'function') loadHostsTab();
  }).catch(function(e) {
    btn.disabled = false; btn.textContent = '✦ 重試 ✦';
    console.error('[batch-onboard] error:', e);
    alert('上線過程出錯:\n\n' + (e && e.message ? e.message : e) +
          '\n\n排查建議:\n1. F12 Console 看完整錯誤\n2. F12 Network 看 batch-onboard 的 Response\n3. 在系統管理 → 系統健康檢查 跑一次');
  });
}

function boRenderResults(results) {
  var tbody = document.getElementById('bo-result-tbody');
  tbody.innerHTML = '';
  document.getElementById('bo-result-summary').innerHTML = '共 '+results.length+' 台';
  results.forEach(function(r) {
    var addedIcon = r.added ? '<span style="color:var(--g1)">新增</span>'
                  : '<span style="color:var(--c3)">已存在</span>';
    var osIcon = r.probe_status === 'ok'
        ? '<span style="color:var(--g1)" title="'+(r.os||'')+'">✓ '+(r.os_group||'')+'</span>'
        : r.probe_status === 'failed' ? '<span style="color:var(--red)" title="'+(r.error||'')+'">✗</span>'
        : '<span style="color:var(--c3)">-</span>';
    var safeId = (r.hostname||'').replace(/[^a-zA-Z0-9_-]/g,'_');
    var inspIcon = '<span class="bo-insp-' + safeId + '" style="color:var(--c3)">⏳</span>';
    var actionHtml = '<a href="/" onclick="closeBatchOnboardModal();return true;" style="font-size:10px;color:var(--primary);">看主機</a>';
    var tr = document.createElement('tr');
    tr.innerHTML = '<td style="padding:4px 8px;border:1px solid var(--border);">' + r.hostname + '</td>'
      ...
      + '<td style="padding:4px 8px;border:1px solid var(--border);text-align:center;">' + osIcon + '</td>'
      + '<td style="padding:4px 8px;border:1px solid var(--border);text-align:center;">' + inspIcon + '</td>'
      + '<td style="padding:4px 8px;border:1px solid var(--border);">' + actionHtml + '</td>';
    tbody.appendChild(tr);
  });
}

function pollOnboardStatus(jobId, hostnames) {
  var btn = document.getElementById('bo-start-btn');
  var attempts = 0, maxAttempts = 60;
  var poll = function() {
    attempts++;
    var elapsed = attempts * 5;
    document.getElementById('bo-step-inspect').innerHTML =
      '⏳ 首次巡檢進行中... ('+elapsed+'s)';
    fetch('/api/admin/hosts/onboard-status?job_id=' + encodeURIComponent(jobId))
      .then(function(r){ return r.json(); }).then(function(res) {
        if (!res || !res.success) { setTimeout(poll, 5000); return; }
        (res.hosts || []).forEach(function(h) {
          var safeId = (h.hostname||'').replace(/[^a-zA-Z0-9_-]/g, '_');
          var el = document.querySelector('.bo-insp-' + safeId);
          if (el) {
            if (h.has_today_data) { el.innerHTML = '✓'; el.style.color = 'var(--g1)'; }
            else if (h.failed) { el.innerHTML = '✗'; el.style.color = 'var(--red)'; }
            else { el.innerHTML = '⏳'; }
          }
        });
        if (res.ready || attempts >= maxAttempts) {
          var doneCount = (res.hosts||[]).filter(function(h){return h.has_today_data;}).length;
          document.getElementById('bo-step-inspect').innerHTML =
            '✓ 首次巡檢完成 ('+doneCount+'/'+hostnames.length+')';
          document.getElementById('bo-step-inspect').style.color = 'var(--g1)';
          btn.disabled = false; btn.textContent = '✦ 完成 ✦';
        } else {
...
function startSubnetScan() {
  var cidr = (document.getElementById('bo-scan-cidr').value || '').trim();
  if (!cidr) { document.getElementById('bo-scan-cidr').focus(); return; }
  var btn = document.getElementById('bo-scan-btn');
  var st  = document.getElementById('bo-scan-status');
  btn.disabled = true; btn.textContent = '掃描中...';
  st.textContent = '正在 ping sweep ' + cidr + '，請稍候（約 10~60 秒）...';
  document.getElementById('bo-scan-result').style.display = 'none';

  fetch('/api/admin/hosts/scan-subnet', {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({cidr: cidr})
  }).then(function(r){ return r.json(); }).then(function(res) {
    btn.disabled = false; btn.textContent = '掃描';
    if (!res.success) { st.textContent = '錯誤: ' + res.error; return; }
    st.textContent = '掃描完成，工具: ' + res.tool + '，找到 ' + res.count + ' 台存活主機';
    document.getElementById('bo-scan-count').textContent =
      '找到 ' + res.count + ' 台 | 已在系統: ' +
      res.hosts.filter(function(h){ return h.in_db; }).length + ' 台';

    // v3.17.18.0: count by os_group
    var osStats = {linux:0, aix:0, windows:0, as400:0, unknown:0};
    (res.hosts || []).forEach(function(h){
      var g = h.os_group || 'unknown';
      if (osStats[g] === undefined) g = 'unknown';
      osStats[g]++;
    });
    var osStatsHtml = ' | OS: ' +
      (osStats.linux ? '<span style="color:#4AB234">Linux '+osStats.linux+'</span> ' : '') +
      (osStats.aix ? '<span style="color:#9C27B0">AIX '+osStats.aix+'</span> ' : '') +
      (osStats.windows ? '<span style="color:#0078D4">Windows '+osStats.windows+'</span> ' : '') +
      (osStats.as400 ? '<span style="color:#333">AS400 '+osStats.as400+'</span> ' : '') +
      (osStats.unknown ? '<span style="color:var(--c4)">未知 '+osStats.unknown+'</span>' : '');
    document.getElementById('bo-scan-count').innerHTML =
      '找到 ' + res.count + ' 台 | 已在系統: ' +
      res.hosts.filter(function(h){ return h.in_db; }).length + ' 台' + osStatsHtml;

    var tbody = document.getElementById('bo-scan-tbody');
    tbody.innerHTML = '';
    var osColors = {linux:'#4AB234', aix:'#9C27B0', windows:'#0078D4', as400:'#333', unknown:'#999'};
    (res.hosts || []).forEach(function(h) {
      var tr = document.createElement('tr');
      var badge = h.in_db
        ? '<span style="color:var(--c3);font-size:10px;">已在系統</span>'
        : '<span style="color:var(--g1);font-size:10px;">未加入</span>';
      var osColor = osColors[h.os_group||'unknown'] || '#999';
      var osBadge = '<span style="background:'+osColor+';color:white;font-size:10px;padding:2px 6px;border-radius:3px;">'+(h.os_hint||'Unknown')+'</span>';
      tr.innerHTML =
        '<td style="padding:4px 8px;border-bottom:1px solid var(--border);text-align:center;">' +
          '<input type="checkbox" class="bo-scan-cb" data-ip="'+h.ip+'" data-hn="'+(h.hostname||'')+'" data-osg="'+(h.os_group||'')+'" data-osh="'+(h.os_hint||'')+'"></td>' +
        '<td style="padding:4px 8px;border-bottom:1px solid var(--border);font-family:monospace;">'+h.ip+'</td>' +
        '<td style="padding:4px 8px;border-bottom:1px solid var(--border);">'+(h.hostname||'<span style="color:var(--c4)">未知</span>')+'</td>' +
        '<td style="padding:4px 8px;border-bottom:1px solid var(--border);">'+osBadge+'</td>' +
        '<td style="padding:4px 8px;border-bottom:1px solid var(--border);">'+badge+'</td>';
      tbody.appendChild(tr);
    });
    document.getElementById('bo-scan-result').style.display = '';
  }).catch(function(e) {
    btn.disabled = false; btn.textContent = '掃描';
    st.textContent = '錯誤: ' + e;
  });
...
function boScanToggleAll(cb) {
  document.querySelectorAll('.bo-scan-cb').forEach(function(c){ c.checked = cb.checked; });
}
function boScanSelectNew() {
  document.querySelectorAll('.bo-scan-cb').forEach(function(c) {
    var row = c.closest('tr');
    c.checked = row && row.querySelector('td:last-child span') &&
                row.querySelector('td:last-child span').textContent.includes('未加入');
  });
}
function boScanAddSelected() {
  var arr = [];
  document.querySelectorAll('.bo-scan-cb:checked').forEach(function(c) {
    arr.push({
      hostname: c.dataset.hn || c.dataset.ip || '',
      ip: c.dataset.ip || '',
      os_hint: c.dataset.osh || '',
      os_group: c.dataset.osg || '',
      source: 'scan'
    });
  });
  if (!arr.length) { alert('請先勾選主機'); return; }
  var added = boAddToCandidates(arr);
  alert('已加入候選清單 ' + added + ' 台' + (added < arr.length ? '（' + (arr.length - added) + ' 台已存在）' : ''));
}
// ===== END Subnet Scan =====
