...
function boBatchTab(tab) {
  ...
  document.getElementById('bo-panel-scan').style.display  = tab === 'scan'  ? '' : 'none';
  ...
  document.getElementById('bo-tab-scan').style.background  = tab === 'scan'  ? 'var(--primary)' : 'var(--c4)';
  document.getElementById('bo-tab-scan').style.color       = tab === 'scan'  ? 'white' : 'var(--c2)';
}
...
function boAddSingle() {
  ...
  var ta = document.getElementById('bo-paste-input');
  var line = hn + (ip ? '  ' + ip : '');
    ...
    (res.hosts || []).forEach(function(h) {
      ...
      tr.innerHTML =
        '<td style="padding:4px 8px;border-bottom:1px solid var(--border);text-align:center;">' +
          '<input type="checkbox" class="bo-scan-cb" data-ip="'+h.ip+'" data-hn="'+(h.hostname||'')+'" data-osg="'+(h.os_group||'')+'"></td>' +
...
function boScanAddSelected() {
  ...
  document.querySelectorAll('.bo-scan-cb:checked').forEach(function(c) {
    var hn = c.dataset.hn || '';
    var ip = c.dataset.ip || '';
    lines.push((hn || ip) + (ip && hn ? '  ' + ip : ''));
  });
  if (!lines.length) { alert('請先勾選主機'); return; }
  boBatchTab('paste');
  ta.value = (ta.value ? ta.value.trimEnd() + '\n' : '') + lines.join('\n') + '\n';
}
// ===== END Subnet Scan =====
