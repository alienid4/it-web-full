#!/bin/bash
# v3.17.19.1 - Host Delete Enhancement
#
# Changes:
#   - DELETE /api/admin/hosts/<hostname> 自動重建 inventory
#   - 加 ?purge=1 參數可同時清巡檢/nmon/帳號/TWGCB/套件 歷史
#   - UI 刪除按鈕改 2-step confirm: 是否同時清歷史
#   - generate_inventory.py 讀 INSPECTION_HOME env var (適配 /opt vs /seclog/AI 雙路徑)
#
# Baseline: v3.17.19.0
...
echo
echo "========================================"
echo "  v3.17.19.1 install complete"
echo "========================================"
echo "刪除主機新流程:"
echo "  主機管理 → 任一主機 row → [刪除]"
echo "  Step 1 confirm: 確認刪除主機"
echo "  Step 2 confirm: 是否同時清歷史 (inspections/nmon/...)"
echo "  完成後自動重建 inventory"
