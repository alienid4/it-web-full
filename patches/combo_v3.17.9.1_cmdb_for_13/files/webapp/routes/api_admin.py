...
def system_status():
    ...
    disk_seclog = shutil.disk_usage("/seclog") if os.path.exists("/seclog") else disk
...
def create_backup():
    ...
    try:
        subprocess.run(["tar", "czf", path, "-C", "/seclog/AI", "inspection/"], check=True, timeout=120)
...
def restore_backup(name):
    ...
    try:
        subprocess.run(["tar", "xzf", path, "-C", "/seclog/AI"], check=True, timeout=120)
...
@bp.route("/recon/upload", methods=["POST"])
@admin_required
def recon_upload():
...
@bp.route("/hosts/<hostname>/history", methods=["GET"])
@login_required
...
@bp.route("/hosts/duplicates", methods=["GET"])
@admin_required
...
def ssh_batch_deploy():
    ...
    inventory = "/seclog/AI/inspection/ansible/inventory/hosts.yml"
