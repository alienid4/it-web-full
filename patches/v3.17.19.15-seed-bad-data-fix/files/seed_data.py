...
def import_inspections():
    ...
    import re
    # v3.17.19.15: 只認帶 timestamp prefix 的檔 (YYYYMMDD_HHMMSS_hostname.json)
    # 排除 twgcb_*.json / network_*.json 這類 misc result 檔
    ...
    for fp in files:
        ...
        if bn.endswith("_report.json"):
            continue
        if not ts_re.match(bn):
            skipped += 1
            continue
        try:
            with open(fp, encoding="utf-8") as f:
        ...
        ts = bn.split("_")[0] + "_" + bn.split("_")[1]
